...
if [ -f .env ]; then
  ...
  while IFS='=' read -r key value || [ -n "$key" ]; do
    ...
    # Strip trailing comments and whitespace from value
...
ssh -p "$VPS_PORT" -t "$VPS_USER@$VPS_HOST" "
    ...
    # C. Run Strapi Import as Root
    # Elevated permissions are required to handle the directory renames Strapi 5 performs
    echo \"   >> 🐳 Running import inside \$CONTAINER_NAME as root...\"
