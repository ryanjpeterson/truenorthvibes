...
export default function PostList({ posts, categories = [], pagination }: PostListProps) {
  ...
  const scrollToTop = () => {
    if (listTopRef.current) {
      setTimeout(() => { listTopRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 100);
    }
  };
    ...
    <div ref={listTopRef} className="space-y-12 scroll-mt-24">
      ...
      {(currentCategory || currentSearch) && (
        <div className="flex items-center gap-2 text-sm text-gray-500 -mt-6 ml-1">
          ...
          {currentCategory && <span className="font-medium text-blue-800 bg-blue-50 px-2 py-0.5 rounded">Category: {currentCategory}</span>}
          {currentSearch && <span className="font-medium text-blue-800 bg-blue-50 px-2 py-0.5 rounded">Search: &quot;{currentSearch}&quot;</span>}
      ...
      {posts.length > 0 ? (
          ...
          {pagination && pagination.pageCount > 1 && (
            <div className="flex justify-center items-center gap-4 mt-12">
              <button
                ...
                className={`p-2 rounded-full border ${pagination.page === 1 ? 'text-gray-300 border-gray-200 cursor-not-allowed' : 'text-blue-600 border-blue-200 hover:bg-blue-50'}`}
              >
                <ChevronLeft size={20} />
              </button>
              <span className="text-sm font-medium text-gray-600">Page {pagination.page} of {pagination.pageCount}</span>
              <button
                onClick={() => handlePageChange(pagination.page + 1)}
                disabled={pagination.page === pagination.pageCount}
                className={`p-2 rounded-full border ${pagination.page === pagination.pageCount ? 'text-gray-300 border-gray-200 cursor-not-allowed' : 'text-blue-600 border-blue-200 hover:bg-blue-50'}`}
              >
        ...
        <div className="text-center py-20 bg-gray-50 rounded-xl border border-dashed border-gray-200">
          ...
          <button onClick={clearAll} className="mt-4 text-blue-600 hover:underline font-medium">Clear filters</button>
        </div>
      )}
    </div>
  );
